...
int validate_struct(Student cur){
    ...
    for(size_t i = 0; i<5; i++){
        if (cur.marks[i]>5 || cur.marks[i]<2) return -1;
    }
...
int read_from_file(FILE * filein, Student ** students, int * len){
    ...
    *students = malloc(sizeof(Student)* cur_len);
    ...
    while (fscanf(filein, "%u %255s %255s %255s %u %u %u %u %u", &cur.id, \
...
int write_to_file_after_sort(FILE *fileout, Student **students, int len) {
    for (int i = 0; i < len; i++) {
        double sr = (students[i]->marks[0] + students[i]->marks[1] + 
                     students[i]->marks[2] + students[i]->marks[3] + 
                     students[i]->marks[4]) / 5.0;
...
int write_to_file_above_sr(FILE * fileout, Student *cur ,int len, double sr){
    for (size_t i = 0; i<len; i++){
        double cur_sr = (cur[i].marks[0] + cur[i].marks[1] + cur[i].marks[2] + cur[i].marks[3] + cur[i].marks[4])/5.0;
        if (cur_sr>sr){
...
double find_sr(Student * cur, int len){
    double res = 0;
    for (size_t i = 0; i<len; i++){
        res+=(cur[i].marks[0] + cur[i].marks[1] + cur[i].marks[2] + cur[i].marks[3] + cur[i].marks[4])/5.0;
    }
    res/=len;
    return res;
}
