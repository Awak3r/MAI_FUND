...
// ============= Тесты для func_a (%Ro - римские цифры) =============
...
TEST_F(CustomPrintfTest, FuncB_SmallNumbers) {
    ...
    EXPECT_TRUE(result[strlen(result)-1] == '1'); // Проверяем единицу в конце
    free(result);
}
...
// ============= Тесты для func_c (%Cv - нижний регистр) =============
...
// ============= Тесты для func_d (%CV - верхний регистр) =============
...
// ============= Тесты для func_h (%mu - дамп unsigned int) =============
...
// ============= Тесты для func_i (%md - дамп double) =============

TEST_F(CustomPrintfTest, FuncI_DoublePositive) {
    char* result = func_i(4.25);
    EXPECT_NE(result, nullptr);
    EXPECT_EQ(strlen(result), 71); // 8 байт * 8 бит + 7 пробелов
    free(result);
}
...
// ============= Тесты для func_j (%mf - дамп float) =============

TEST_F(CustomPrintfTest, FuncJ_Float) {
    char* result = func_j(3.75f);
    EXPECT_NE(result, nullptr);
    EXPECT_EQ(strlen(result), 35); // 4 байта * 8 бит + 3 пробела
    free(result);
}
